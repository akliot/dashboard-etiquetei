#!/usr/bin/env python3
"""
Testes unitários para o pipeline omie_sync_bq.py (Etiquetei).
...
class TestIsSafeSql(unittest.TestCase):
    """Testa validação de SQL (padrão de segurança)."""
    ...
    def _check(self, sql: str) -> bool:
        ...
        for word in dangerous:
            if word in words:
                return False
        if "etiquetei" not in sql:
            return False
        return True

    def test_select_valido(self):
        self.assertTrue(self._check("SELECT * FROM `dashboard-koti-omie.etiquetei.lancamentos`"))

    def test_insert_bloqueado(self):
        self.assertFalse(self._check("INSERT INTO `etiquetei.lancamentos` VALUES (1)"))

    def test_delete_bloqueado(self):
        self.assertFalse(self._check("DELETE FROM `etiquetei.lancamentos`"))

    def test_drop_bloqueado(self):
        self.assertFalse(self._check("DROP TABLE `etiquetei.lancamentos`"))
    ...
    def test_update_bloqueado(self):
        self.assertFalse(self._check("UPDATE `etiquetei.lancamentos` SET valor=0"))
