...
def merge_to_bq(
    ...
    # 1. Carregar na staging (WRITE_TRUNCATE) — usar schema da tabela alvo
    try:
...
def coletar_lancamentos(
    ...
    for r in cr_raw:
        if r.get("id_conta_corrente") in CONTAS_IGNORAR:  # ⚡ KOTI-SPECIFIC
            ignorados_cr += 1
            continue
        cat_cod = str(r.get("codigo_categoria", "") or "")
        ...
        cat_grupo = str(".".join(str(cat_cod).split(".")[:2])) if cat_cod else None
